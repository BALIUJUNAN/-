 ...
 * 模块: 核心(哈希表/文件IO)、会员、促销、套装、批次、财务、门店、调拨、供应商结算、储值卡
 */
...
// 积分规则（阶梯制度）
#define POINTS_PER_YUAN_BASE     1     // 基础：1元 = 1积分
#define POINTS_THRESHOLD_100     100   // 100元门槛
#define POINTS_PER_YUAN_LEVEL1  2      // 100元以上：1元 = 2积分
#define POINTS_THRESHOLD_500    500   // 500元门槛
#define POINTS_PER_YUAN_LEVEL2  3      // 500元以上：1元 = 3积分
#define POINTS_THRESHOLD_1000   1000  // 1000元门槛
#define POINTS_PER_YUAN_LEVEL3  5      // 1000元以上：1元 = 5积分
#define POINTS_REDEEM_RATE   100  // 100积分抵1元

// 储值卡支付折扣率（基于会员等级）
#define VIP_DISCOUNT_NORMAL   0.00f   // 普通会员：无折扣
#define VIP_DISCOUNT_SILVER   0.02f   // 银卡会员：2%折扣
#define VIP_DISCOUNT_GOLD     0.05f   // 金卡会员：5%折扣
#define VIP_DISCOUNT_DIAMOND  0.10f   // 钻石会员：10%折扣
...
// 综合折扣信息
typedef struct DiscountInfo {
    float total_discount_rate;      // 总折扣百分比
    float item_discount_rate;        // 单品折扣百分比
    float member_discount_rate;      // 会员折扣百分比
    float override_discount;          // 满减金额
    float total_discount_amount;      // 总折扣金额
    int has_item_discount;           // 是否有单品折扣
    int has_member_discount;         // 是否有会员折扣
    int has_override;                // 是否有满减
} DiscountInfo;
...
// 会员全局变量
extern HashTable *g_member_phone_hash;
...
extern HashTable *g_combo_barcode_hash;
...
float get_vip_card_discount(Member *member);
float get_vip_card_discount_percent(Member *member);
...
int is_promotion_valid(Promotion *p);
