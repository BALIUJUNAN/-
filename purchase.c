...
int load_purchases(void) {
    ...
    while (fgets(line, sizeof(line), fp)) {
        ...
        char *token;
        char *saveptr;
        token = strtok_r(line, "|", &saveptr); pur->id = token ? atoi(token) : 0;
        token = strtok_r(NULL, "|", &saveptr); if (token) strncpy(pur->supplier_id, token, 19);
        token = strtok_r(NULL, "|", &saveptr); if (token) strncpy(pur->supplier_name, token, MAX_NAME_LEN-1);
        token = strtok_r(NULL, "|", &saveptr); pur->creator_id = token ? atoi(token) : 0;
        token = strtok_r(NULL, "|", &saveptr); pur->approver_id = token ? atoi(token) : 0;
        token = strtok_r(NULL, "|", &saveptr); pur->status = token ? atoi(token) : 0;
        token = strtok_r(NULL, "|", &saveptr); pur->total_amount = token ? atof(token) : 0.0f;
        token = strtok_r(NULL, "|", &saveptr); pur->created_at = token ? (time_t)atoll(token) : 0;
        token = strtok_r(NULL, "|", &saveptr); pur->approved_at = token ? (time_t)atoll(token) : 0;
        token = strtok_r(NULL, "|", &saveptr); pur->completed_at = token ? (time_t)atoll(token) : 0;
...
int load_purchase_items(void) {
    ...
    while (fgets(line, sizeof(line), fp)) {
        ...
        char *token;
        char *saveptr;
        token = strtok_r(line, "|", &saveptr); item->id = token ? atoi(token) : 0;
        token = strtok_r(NULL, "|", &saveptr); item->purchase_id = token ? atoi(token) : 0;
        ...
        token = strtok_r(NULL, "|", &saveptr); item->quantity = token ? atof(token) : 0.0f;
        token = strtok_r(NULL, "|", &saveptr); item->price = token ? atof(token) : 0.0f;
        token = strtok_r(NULL, "|", &saveptr); item->received_qty = token ? atof(token) : 0.0f;
