...
float get_vip_card_discount(Member *member) {
    if (!member) return 1.0f;
    switch (member->level) {
        case MEMBER_LEVEL_NORMAL:  return (1.0f - VIP_DISCOUNT_NORMAL);   // 1.00
        case MEMBER_LEVEL_SILVER:  return (1.0f - VIP_DISCOUNT_SILVER);   // 0.98
        case MEMBER_LEVEL_GOLD:    return (1.0f - VIP_DISCOUNT_GOLD);     // 0.95
        case MEMBER_LEVEL_DIAMOND: return (1.0f - VIP_DISCOUNT_DIAMOND); // 0.90
        default: return 1.0f;
    }
}
...
float get_vip_card_discount_percent(Member *member) {
    if (!member) return 0.0f;
    switch (member->level) {
        case MEMBER_LEVEL_NORMAL:  return VIP_DISCOUNT_NORMAL * 100;   // 0%
        case MEMBER_LEVEL_SILVER:  return VIP_DISCOUNT_SILVER * 100;   // 2%
        case MEMBER_LEVEL_GOLD:    return VIP_DISCOUNT_GOLD * 100;     // 5%
        case MEMBER_LEVEL_DIAMOND: return VIP_DISCOUNT_DIAMOND * 100;  // 10%
        default: return 0.0f;
    }
}
...
int add_points(Member *member, float amount) {
    int earned = calculate_ladder_points(amount);
...
int create_combo(ProductCombo *combo) {
    ...
    *new_combo = *combo;
    ...
    if (strlen(combo->barcode) > 0 && g_combo_barcode_hash) {
        hash_insert(g_combo_barcode_hash, combo->barcode, new_combo);
    }
...
ProductCombo* find_combo_by_barcode(const char *barcode) {
    ...
    if (g_combo_barcode_hash) {
        return (ProductCombo*)hash_search(g_combo_barcode_hash, barcode);
    }
...
int load_combos(void) {
    ...
    while (fgets(line, sizeof(line), fp)) {
        ...
        if (strlen(combo->barcode) > 0) {
            hash_insert(g_combo_barcode_hash, combo->barcode, combo);
        }
