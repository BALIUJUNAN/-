 ...
 * │    - 热销商品 TOP N（从 sales.txt + sale_item.txt 关联统计）          │
 * │    - ASCII 柱状图可视化                                              │
 * ├─────────────────────────────────────────────────────────────────────┤
 * │ 3. 日期解析工具                                                      │
...
static int printer_save_to_file(const Receipt *receipt) {
    ...
    snprintf(filepath, sizeof(filepath), "%s/receipt_%04d%02d%02d_%08d.txt",
             OUTPUT_DIR,
...
// [generate_dashboard_metrics removed]
// [print_dashboard removed]
